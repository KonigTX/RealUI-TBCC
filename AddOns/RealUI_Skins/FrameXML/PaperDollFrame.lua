...
do --[[ FrameXML\PaperDollFrame.lua ]]
    ...
    function Hook.PaperDollItemSlotButton_Update(self)
        ...
        local subicon = self.subicon or _G[self:GetName().."SubIconTexture"]
        local slotTexture = _G[self:GetName().."SlotTexture"]
        if slotTexture then
            slotTexture:Hide()
            slotTexture:SetTexture("")
        end
        local slotBackground = _G[self:GetName().."SlotBackground"]
        ...
        if name:find("^Character") or name:find("^Inspect") then
            self.backgroundTextureName = nil
        end
        if itemLink then
            if icon then
                icon:SetAlpha(1)
...
do --[[ FrameXML\PaperDollFrame.xml ]]
    ...
    local function CreateRegions(Button)
        ...
        local ilvl = _G[Button:GetName().."ItemLevel"]
        if not ilvl then
            ilvl = Button:CreateFontString(name.."ItemLevel", "OVERLAY")
    ...
    -- TBCC: Check if skin functions exist before hooking
    if Skin.PaperDollItemSlotButtonLeftTemplate and type(Skin.PaperDollItemSlotButtonLeftTemplate) == "function" then
    ...
    if Skin.PaperDollItemSlotButtonRightTemplate and type(Skin.PaperDollItemSlotButtonRightTemplate) == "function" then
    ...
    end -- TBCC: End of safety check for PaperDollItemSlotButtonRightTemplate
...
-- TBCC: Check if FrameXML function exists before hooking
if private.FrameXML and private.FrameXML["PaperDollFrame"] and type(private.FrameXML["PaperDollFrame"]) == "function" then
