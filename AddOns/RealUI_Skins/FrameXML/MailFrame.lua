...
do --[[ FrameXML\MailFrame.lua ]]
    local numItems
    -- TBCC: Check if Hook.InboxFrame_Update exists before hooking
    if Hook.InboxFrame_Update and type(Hook.InboxFrame_Update) == "function" then
    ...
    end -- TBCC: End of safety check for Hook.InboxFrame_Update
end
...
do --[[ FrameXML\MailFrame.xml ]]
    ...
    if Skin.MailItemTemplate and type(Skin.MailItemTemplate) == "function" then
...
-- TBCC: Check if MailFrame hook exists before calling
if private.FrameXML and private.FrameXML["MailFrame"] and type(private.FrameXML["MailFrame"]) == "function" then
...
end -- TBCC: End of safety check for MailFrame hook
