...
do -- Button
    ...
    function Skin.FrameTypeButton(Button, OnEnter, OnLeave)
        ...
        if framesOk then    -- not sure when these are ok to use anymore - SetEnabled triggers properly for tabs
            -- TBCC: Check if methods exist before hooking (item slots don't have these)
        ...
        if Button.ClearNormalTexture then
            ...
            -- TBCC: Check if texture methods exist (item slots don't have these)
...
do -- StatusBar
    local function Hook_SetStatusBarTexture(self, asset)
        ...
        -- TBCC: Check if assetColors exists before indexing
        local color = private.assetColors and private.assetColors[asset]
        local color2 = private.assetColors and private.assetColors[asset .. "_2"]
