...
local function UpdateCurrency()
    ...
    -- TBCC: C_CurrencyInfo API doesn't exist in TBC Classic
    -- Currency tracking works differently in TBCC
    if not _G.C_CurrencyInfo or not _G.C_CurrencyInfo.GetCurrencyListSize then
        scanning = nil
        return
    end
    ...
    while i <= limit do
        ...
        if currencyInfo and currencyInfo.isHeader then
...
function Currency:OnInitialize()
    ...
    -- TBCC: ACCOUNT_MONEY event doesn't exist (Warband feature)
    pcall(function() self:RegisterEvent("ACCOUNT_MONEY", UpdateMoney) end)
