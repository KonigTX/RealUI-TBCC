...
function RealUI:InitTutorial()
    -- TBCC: MainHelpPlateButton template may not exist in Classic
    -- Use a simple button instead if the template isn't available
    local templateExists = pcall(function()
        local testFrame = _G.CreateFrame("Button", nil, _G.UIParent, "MainHelpPlateButton")
        testFrame:Hide()
    ...
    if templateExists then
        ...
        -- Fallback: create a simple button without the template
        rTB = _G.CreateFrame("Button", "RealUITutorialButton", _G.UIParent)
        rTB:SetSize(32, 32)
    end
    ...
    -- TBCC: HelpPlate may not exist in Classic, use UIParent as fallback
    local helpPlateParent = _G.HelpPlate or _G.UIParent
    local btnClose = createTextButton("RealUITutorialButtonClose", helpPlateParent)
