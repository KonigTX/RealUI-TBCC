...
-- TBCC: C_AddOns.IsAddOnLoaded doesn't exist in TBCC, use fallback
local IsAddOnLoaded = _G.C_AddOns and _G.C_AddOns.IsAddOnLoaded or _G.IsAddOnLoaded

-- TBCC: SafeSkin wrapper for skin functions that may not exist in TBCC
local function SafeSkin(skinFunc, widget)
    if skinFunc and type(skinFunc) == "function" and widget then
        local success, err = pcall(skinFunc, widget)
        if not success then
...
local function CreateCheckbox(name, parent, label, side, size)
    ...
    -- TBCC: ChatConfigCheckButtonTemplate skin may not exist
    SafeSkin(Skin.ChatConfigCheckButtonTemplate, f)
...
local function CreateTextButton(text, parent, width, height)
    ...
    -- TBCC: UIPanelButtonTemplate skin may not exist
    SafeSkin(Skin.UIPanelButtonTemplate, f)
...
local function CreateWindow(name, width, height)
    ...
    -- TBCC: UIPanelCloseButton skin may not exist
    SafeSkin(Skin.UIPanelCloseButton, f.close)
...
function AddonControl:CreateOptionsFrame()
    ...
    for k, addon in next, RealUIAddOnsOrder do
        -- TBCC: Use local IsAddOnLoaded fallback
