    ...
    if menuItemInfo.isSpacer then
        ...
        if fontString then
            if self:SetCheckedState(menuItemInfo.checked) ~= nil then
                fontString:SetPoint("LEFT", self.checkBox, 20, 0)
            else
                fontString:SetPoint("LEFT", MENU_MARGIN, 0)
            end
        end
    end
...
local function MenuFactory(pool)
    local numActive = pool:GetNumActive()
    local menu = _G.CreateFrame("Frame", "RealUI_MenuFrame"..numActive, _G.UIParent)
