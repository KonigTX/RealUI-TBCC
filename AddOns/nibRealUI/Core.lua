...
-- TBCC: Use C_AddOns.GetAddOnMetadata if available, fallback to GetAddOnMetadata
...
local function UpdateSpec(...)
    -- TBCC: IsPlayerInitialSpec may not exist
    if _G.IsPlayerInitialSpec and _G.IsPlayerInitialSpec() then
    ...
    -- TBCC: C_SpecializationInfo doesn't exist in TBC Classic
    if not _G.C_SpecializationInfo or not _G.C_SpecializationInfo.GetSpecialization then
        return
    end

    local specInfo = RealUI.charInfo.specs
    local new = _G.C_SpecializationInfo.GetSpecialization()
    if new and specInfo.current.index ~= new then
...
function RealUI.LoadConfig(app, section, ...)
    ...
    if not configLoaded then
        local reason
        configLoaded, reason = LoadAddOn("nibRealUI_Config")
...
function RealUI:OnInitialize()
    ...
    -- TBCC: PLAYER_SPECIALIZATION_CHANGED doesn't exist in TBC Classic, wrap in pcall
    pcall(function() self:RegisterEvent("PLAYER_SPECIALIZATION_CHANGED", UpdateSpec) end)
    self:RegisterEvent("PLAYER_TALENT_UPDATE", UpdateSpec)
    -- TBCC: TRAIT_CONFIG_UPDATED doesn't exist in TBC Classic, wrap in pcall
    pcall(function() self:RegisterEvent("TRAIT_CONFIG_UPDATED", UpdateSpec) end)
    ...
    self:RegisterChatCommand(
        "rl",
        function()
            -- TBCC: Use C_UI.Reload if available, fallback to ReloadUI
    ...
    -- Check AccountStatus
    -- TBCC: GameLimitedMode_IsActive may not exist
    ...
    -- TBCC: C_AddOnProfiler doesn't exist in TBC Classic
    if _G.C_AddOnProfiler and _G.C_AddOnProfiler.IsEnabled then
...
-- NOTE: Previous Bartender4 UpdateStates hook was removed - it caused action bar taint
-- The original StateBar.lua already calls self:ForAll("UpdateState") at the end of UpdateStates
