...
-- TBCC: GameTooltipTemplate is a template string, not a function
if Aurora.Skin.GameTooltipTemplate and type(Aurora.Skin.GameTooltipTemplate) == "function" then
...
function Infobar:AddBlock(name, dataObj, blockInfo)
    ...
    if dataObj.events then
        ...
        for i = 1, #dataObj.events do
            -- TBCC: Some events don't exist, wrap in pcall
            pcall(function() block:RegisterEvent(dataObj.events[i]) end)
        end
    end
...
function Infobar:SettingsUpdate(setting, block)
    ...
    if setting == "fontSize" then
        blockFont.size = db.fontSize
        self.frame.left:UpdateBlocks(true)
        self.frame.right:UpdateBlocks(true)
        return
    elseif setting == "statusBar" or setting == "HideStatusBarMaxLevel" then
...
function Infobar:OnEnable()
    ...
    blockFont = {
        ...
        size = db.fontSize or RealUI.Round(BAR_HEIGHT * 0.6),
