...
-- TBCC: Check if garrison API exists before using it
local garrisonTypeSL
if _G.Enum.GarrisonType then
    garrisonTypeSL = _G.Enum.GarrisonType["Type_9_0_Garrison"]
end

local function UpdateGarrisonButton(isTop, isLeft)
    -- TBCC: Check if C_Garrison exists before calling
...
local function AddPOIsForZone(zoneInfo, numNumericQuests)
    -- TBCC: This entire function relies on Retail-specific APIs that don't exist in Classic
    -- Required APIs: GetQuestsOnMapCached, C_QuestLog.IsQuestCalling, C_SuperTrack, QuestMapFrame, QuestScrollFrame
    if not (_G.C_SuperTrack and _G.QuestMapFrame and _G.QuestScrollFrame) then
        return numNumericQuests
    end

    local quests = _G.GetQuestsOnMapCached and _G.GetQuestsOnMapCached(zoneInfo.mapID) or (_G.C_QuestLog and _G.C_QuestLog.GetQuestsOnMap and _G.C_QuestLog.GetQuestsOnMap(zoneInfo.mapID))
    if not quests then return numNumericQuests end

    for _, questInfo in next, quests do
        local questID = questInfo.questID
        -- TBCC: C_QuestLog.IsQuestCalling may not exist
        local isQuestCalling = _G.C_QuestLog.IsQuestCalling and _G.C_QuestLog.IsQuestCalling(questID)
        if not isQuestCalling then
            local questLogInfo = _G.C_QuestLog.GetInfo(_G.C_QuestLog.GetLogIndexForQuestID(questID))
            if not questLogInfo then return numNumericQuests end
            local isSuperTracked = _G.C_SuperTrack.GetSuperTrackedQuestID and _G.C_SuperTrack.GetSuperTrackedQuestID() == questID
            ...
            if (not questLogInfo.isHidden and questLogInfo.hasLocalPOI) or isSuperTracked then
                ...
                if xCoord and yCoord and instanceID then
                    ...
                    local poiButton = _G.QuestMapFrame.QuestsFrame and _G.QuestMapFrame.QuestsFrame.Contents and _G.QuestMapFrame.QuestsFrame.Contents:FindButtonByQuestID(questID)
                    if not poiButton and _G.QuestScrollFrame and _G.QuestScrollFrame.Contents then
                    ...
                    if poiButton then
                        if (_G.QuestUtils_IsQuestWatched and _G.QuestUtils_IsQuestWatched(questID)) or not db.poi.watchedOnly then
                            poiButton.xCoord = xCoord
                            poiButton.yCoord = yCoord
                            poiButton.instanceID = instanceID
                            if isSuperTracked and _G.QuestPOI_SelectButton then
...
function MinimapAdv:LootSpecUpdate()
    ...
    if (instanceType == "party" or instanceType == "raid") then
        ...
        local lootSpecName = RealUI.GetCurrentLootSpecName and RealUI.GetCurrentLootSpecName()
        if not lootSpecName or lootSpecName == "" then
            lootSpecName = _G.NONE or ""
        end
        MMFrames.info.LootSpec.text:SetFormattedText("|c%s%s:|r %s", Color.blue.colorStr, _G.LOOT, lootSpecName)
...
-- TBCC: C_Garrison doesn't exist in TBCC (WoD feature), wrap in nil check
local garrisonTypeWoD, followerTypeWoD, currencyId, categoryInfo
if _G.C_Garrison then
    garrisonTypeWoD = _G.Enum.GarrisonType["Type_7_0_Garrison"]
    followerTypeWoD = _G.Enum.GarrisonFollowerType["FollowerType_7_0_GarrisonFollower"]

    currencyId = _G.C_Garrison.GetCurrencyTypes(garrisonTypeWoD)
    categoryInfo = {}
    do -- by nebula
...
local function Garrison_OnEnter(self)
    ...
    -- TBCC: Check if C_Garrison exists before using it
    if _G.C_Garrison and _G.C_Garrison.GetLandingPageGarrisonType() == garrisonTypeWoD then
...
local function CreateFrames()
        ...
        for id = 1, count do
            ...
            local info = {
                ...
                func = function(dialog, arg1, arg2, selected)
                    if _G.MinimapUtil and _G.MinimapUtil.SetTrackingFilterByFilterIndex then
                        _G.MinimapUtil.SetTrackingFilterByFilterIndex(id, selected)
                    elseif _G.C_Minimap and _G.C_Minimap.SetTracking then
                        _G.C_Minimap.SetTracking(id, selected)
                    end
    ...
    -- TBCC: AddonCompartmentFrame doesn't exist in TBC Classic
    if _G.AddonCompartmentFrame then
        if Skin.DropdownButton and type(Skin.DropdownButton) == "function" then
...
local function SetUpMinimapFrame()
   ...
   -- -- Make it square
    ...
    -- TBCC: Zoom buttons are separate frames (MinimapZoomIn/MinimapZoomOut), not Minimap children
    ...
    -- TBCC: MinimapCluster structure is different, check for existence
    if _G.MinimapCluster.BorderTop then _G.MinimapCluster.BorderTop:Hide() end
    if _G.MinimapCluster.Tracking then
        if _G.MinimapCluster.Tracking.Button then _G.MinimapCluster.Tracking.Button:Hide() end
        if _G.MinimapCluster.Tracking.Background then _G.MinimapCluster.Tracking.Background:Hide() end
    end
    if _G.MinimapCluster.ZoneTextButton then _G.MinimapCluster.ZoneTextButton:Hide() end
    -- TBCC: Hide classic minimap elements (the circular border and overlays)
    ...
    if _G.MiniMapTracking then _G.MiniMapTracking:Hide() end
    if _G.MiniMapWorldMapButton then _G.MiniMapWorldMapButton:Hide() end
    if _G.GameTimeFrame then _G.GameTimeFrame:Hide() end
    if _G.MiniMapMailFrame then _G.MiniMapMailFrame:Hide() end
    if _G.MiniMapBattlefieldFrame then _G.MiniMapBattlefieldFrame:Hide() end
    if _G.MiniMapLFGFrame then _G.MiniMapLFGFrame:Hide() end
    if _G.MinimapBackdrop then _G.MinimapBackdrop:Hide() end
    if _G.MinimapNorthTag then _G.MinimapNorthTag:SetAlpha(0) end
    ...
    -- TBCC: QueueStatusButton may not exist in TBCC
    ...
    -- TBCC: MinimapCluster structure differs between Retail and TBCC
    ...
    if _G.MinimapCluster and _G.MinimapCluster.IndicatorFrame then
...
function MinimapAdv:OnEnable()
    if not HBD then
        -- TBCC: Use C_AddOns.LoadAddOn if available, fallback to LoadAddOn
        local LoadAddOn = _G.C_AddOns and _G.C_AddOns.LoadAddOn or _G.LoadAddOn
        LoadAddOn("HereBeDragons")
