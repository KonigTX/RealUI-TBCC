...
function DragEmAll:HookFrame(frameName, children)
    ...
    if frameName == "FlightMapFrame" then
        frame:HookScript("OnShow", function()
            DragEmAll:ApplyFlightMapScale()
        end)
        self:ApplyFlightMapScale()
    end
...
function DragEmAll:ApplyFlightMapScale()
    local scale = self.db.profile.flightMapScale or 1
...
function DragEmAll:OnEnable()
    ...
    if ColorPickerFrame then
        ...
        -- TBCC: ColorPickerFrame.Header may not exist in TBCC
