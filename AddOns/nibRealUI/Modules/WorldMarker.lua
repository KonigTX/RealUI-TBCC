...
function WorldMarker:OnInitialize()
    ...
    for index = 1, #MARKER_COLORS do
        ...
        -- TBCC: Protected frame anchoring may fail, wrap in pcall
        if index == 1 then
            pcall(function() button:SetPoint("TOPLEFT") end)
        else
            pcall(function() button:SetPoint("TOPLEFT", frame[index - 1], "BOTTOMLEFT", 0, 0) end)
        end
    ...
    pcall(function() button:SetPoint("TOPLEFT", frame[#frame], "BOTTOMLEFT", 0, 0) end)
