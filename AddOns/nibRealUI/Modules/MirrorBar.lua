...
local function HideDefaultMirrorTimers()
    for i = 1, _G.MIRRORTIMER_NUMTIMERS or 3 do
        ...
        if timer then
            ...
            timer:SetAlpha(0)
            timer:Hide()
            if not timer._realuiHidden then
                timer:HookScript("OnShow", function(self) self:Hide() end)
                timer._realuiHidden = true
            end
        end
    end
end
...
function MirrorBar:OnEnable()
    ...
    -- Hide Default Mirror Timer
    -- TBCC: MirrorTimerContainer doesn't exist, individual MirrorTimer frames do
    if _G.MirrorTimerContainer then
        _G.MirrorTimerContainer:UnregisterEvent("MIRROR_TIMER_START")
    else
        -- TBCC: Hide default mirror timer frames (MirrorTimer1, MirrorTimer2, MirrorTimer3)
        HideDefaultMirrorTimers()
    end
    if type(_G.MirrorTimer_Show) == "function" then
        _G.hooksecurefunc("MirrorTimer_Show", HideDefaultMirrorTimers)
    end
    if type(_G.MirrorTimer_Start) == "function" then
        _G.hooksecurefunc("MirrorTimer_Start", HideDefaultMirrorTimers)
    end
