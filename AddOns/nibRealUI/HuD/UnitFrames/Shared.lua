...
local function CreatePowerBar(parent, info, isAngled)
    ...
    if isAngled then
        ...
        function Power:UpdateReverse()
            if not ndb.settings then return end
...
local function Shared(self, unit)
    ...
    if unitData.hasCastBars and RealUI:GetModuleEnabled("CastBars") then
        -- TBCC: Check if CastBars module and method exist before calling
        local CastBars = RealUI:GetModule("CastBars", true)
        if CastBars and CastBars.CreateCastBars then
            CastBars:CreateCastBars(self, unit, unitData)
        end
    end
