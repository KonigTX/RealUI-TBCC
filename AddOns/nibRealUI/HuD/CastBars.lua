...
do
    ...
    local function RegisterSpellName(spellID, numticks, isInstant)
        -- TBCC: Wrap spell check in pcall to avoid errors for spells that don't exist in TBCC
        local success, spellInfo = pcall(_G.C_Spell.GetSpellInfo, spellID)
        if not success or not spellInfo then
            return -- Silently skip spells that don't exist in TBCC
        end
    ...
    -- Death Knight
    RegisterSpellName(206931, 3 / 1, true) -- Blooddrinker (Legion+ only)
