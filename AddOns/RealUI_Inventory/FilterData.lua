...
local equipSetItems = {}
-- TBCC: Use simple indices, no C_Bank.FetchNumPurchasedBankTabs
local first, last = 0, (_G.NUM_BAG_SLOTS or 4) + (_G.GetNumBankSlots and _G.GetNumBankSlots() or 0)
...
function private.UpdateEquipSetItems()
    ...
    -- TBCC: Equipment set API might not exist or work differently
    if not _G.C_EquipmentSet or not _G.C_EquipmentSet.GetEquipmentSetIDs then
        return
    end
    ...
    for i = 1, #setIDs do
        ...
        if locations then
            for j = 1, 19 do
                if locations[j] then
                    -- TBCC: EquipmentManager_GetLocationData may not exist
