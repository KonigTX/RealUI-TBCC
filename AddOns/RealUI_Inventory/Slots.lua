...
function ItemSlotMixin:Update()
    ...
    if self == _G.GameTooltip:GetOwner() and self.UpdateTooltip then
        local ok = _G.pcall(self.UpdateTooltip, self)
        if not ok then
            _G.GameTooltip:Hide()
        end
    end
...
function ItemSlotMixin:UpdateCooldown()
    ...
    -- TBCC: Cooldown may not exist on all button templates
...
function InventorySlotMixin:Update()
    ...
    -- TBCC: IsBattlePayItem may not exist or return nil
    local isBattlePay = _G.C_Container.IsBattlePayItem and _G.C_Container.IsBattlePayItem(self:GetBagAndSlot())
    if self.BattlepayItemTexture then
        self.BattlepayItemTexture:SetShown(isBattlePay or false)
    end
end
...
function private.GetSlotTypeForBag(bagID)
    -- TBCC: Bank container is -1, no reagent bank
...
-- TBCC: Use simple numeric indices
local mainBags = {
    [0] = _G.BACKPACK_TOOLTIP, -- Backpack
    [-1] = _G.BANK, -- BANK_CONTAINER
}
local BagSlotMixin = {}
function BagSlotMixin:Init(bagID)
    ...
    if Skin.FrameTypeItemButton and type(Skin.FrameTypeItemButton) == "function" then
        Skin.FrameTypeItemButton(self)
    end
    if self.isBag then
        -- TBCC: Simplified bag slot handling
        if bagID >= 1 and bagID <= (_G.NUM_BAG_SLOTS or 4) then
            -- Regular bags (1-4)
            self.inventorySlot = "Bag"..(bagID - 1).."Slot"
            self.inventoryID, self.fallbackTexture = _G.GetInventorySlotInfo(self.inventorySlot)
        elseif bagID > (_G.NUM_BAG_SLOTS or 4) then
            -- Bank bag slots (5-10 in TBCC)
            local bankSlotID = bagID - (_G.NUM_BAG_SLOTS or 4)
            self.bankSlotID = bankSlotID
            self.inventoryID = _G.BankButtonIDToInvSlotID(bankSlotID, 1)
            self.fallbackTexture = [[Interface\PaperDoll\UI-PaperDoll-Slot-Bag]]
        end
    else
...
function BagSlotMixin:Update()
    if self.isBag then
        ...
        if self.bagType == "bank" then
            -- TBCC: Check if bank slot is purchased
            if self.bankSlotID and self.bankSlotID <= _G.GetNumBankSlots() then
            ...
            -- TBCC: No reagent bag, just use EQUIP_CONTAINER
