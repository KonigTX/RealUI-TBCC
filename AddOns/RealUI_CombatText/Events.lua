...
function eventPrefix.RANGE(eventInfo, ...)
    ...
    -- TBCC: C_Spell doesn't exist, use GetSpellTexture
    if _G.C_Spell and _G.C_Spell.GetSpellInfo then
        ...
        eventInfo.icon = _G.GetSpellTexture(eventInfo.spellID)
    end
...
function eventSuffix.INTERRUPT(eventInfo, extraSpellId, extraSpellName, extraSpellSchool)
    -- TBCC: C_Spell.GetSchoolString doesn't exist, use school name directly or empty string
    local schoolString = ""
    if _G.C_Spell and _G.C_Spell.GetSchoolString then
        schoolString = _G.C_Spell.GetSchoolString(extraSpellSchool)
    end
    eventInfo.string = eventFormat:format(_G.ACTION_SPELL_INTERRUPT, extraSpellName, schoolString)
