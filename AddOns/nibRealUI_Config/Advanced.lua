    ...
    local infobar do
        ...
        infobar = {
            ...
            args = {
                ...
                fontSize = {
                    name = L["Infobar_FontSize"],
                    desc = L["Infobar_FontSizeDesc"],
                    type = "range",
                    min = 8, max = 18, step = 1,
                    get = function() return Infobar.db.profile.fontSize end,
                    set = function(info, value)
                        Infobar.db.profile.fontSize = value
                        Infobar:SettingsUpdate(info[#info])
                    end,
                    order = 54,
                },
...
do -- Skins
    ...
    local classColors do
        ...
        for classToken, color in next, _G.CUSTOM_CLASS_COLORS do
            ...
            -- TBCC: Some classes (DEMONHUNTER, EVOKER) don't exist in TBCC
            -- Only add config option if the class has a localized name
            local className = _G.LOCALIZED_CLASS_NAMES_MALE[classToken]
            if className then
...
do -- Tooltips
	...
	if not Tooltips.db then
		Tooltips.db = RealUI.db:RegisterNamespace("Tooltips", {
			profile = {
				anchor = "CURSOR",
				offsetX = 0,
				offsetY = 0,
    ...
    local function appGet(info)
        return Tooltips.db.profile[info[#info]]
    end
    local function appSet(info, value)
        Tooltips.db.profile[info[#info]] = value
		if Tooltips.UpdateTooltipAnchor then
			Tooltips:UpdateTooltipAnchor()
		end
    end
    ...
    if Tooltips then
        args = {
            showTarget = {
                name = "Show Target",
                desc = "Show the target of the unit in the tooltip.",
                type = "toggle",
                get = appGet,
                set = appSet,
                order = 5,
            ...
            classColors = {
                name = L["Tooltips_ClassColors"],
                desc = L["Tooltips_ClassColorsDesc"],
                type = "toggle",
                get = appGet,
                set = appSet,
                order = 6,
            ...
            guildBrackets = {
                name = L["Tooltips_GuildBrackets"],
                desc = L["Tooltips_GuildBracketsDesc"],
                type = "toggle",
                get = appGet,
                set = appSet,
                order = 7,
            ...
            anchor = {
                name = "Anchor",
                desc = "Sets the anchor point of the main mouse tooltip.",
                type = "select",
                values = {
                    CURSOR = "Cursor",
                    STATIC = "Static",
            ...
            offsets = {
                ...
                type = "group",
                inline = true,
                disabled = function() return Tooltips.db.profile.anchor ~= "CURSOR" end,
                order = 20,
                args = {
                    x = {
                        name = "X Offset",
                        type = "input",
                        width = "half",
                        get = function() return tostring(Tooltips.db.profile.offsetX) end,
                        set = function(info, value)
							local num = tonumber(value)
							if num then
								Tooltips.db.profile.offsetX = num
								if Tooltips.UpdateTooltipAnchor then
									Tooltips:UpdateTooltipAnchor()
								end
							end
                    ...
                    y = {
                        ...
                        type = "input",
                        width = "half",
                        get = function() return tostring(Tooltips.db.profile.offsetY) end,
                        set = function(info, value)
							local num = tonumber(value)
							if num then
								Tooltips.db.profile.offsetY = num
								if Tooltips.UpdateTooltipAnchor then
									Tooltips:UpdateTooltipAnchor()
								end
							end
...
do -- UI Tweaks
    ...
    local flightMap do
        ...
        local DragEmAll = RealUI:GetModule(MODNAME, true)
        if DragEmAll and not DragEmAll.db then
            DragEmAll.db = RealUI.db:RegisterNamespace(MODNAME, {
                profile = {
                    flightMapScale = 1,
                }
            })
        ...
        flightMap = {
            ...
            args = {
                ...
                desc = {
                    name = L["FlightMapDesc"],
                    type = "description",
                    fontSize = "medium",
                    order = 20,
                },
                scale = {
                    name = L["FlightMap_Scale"],
                    desc = L["FlightMap_ScaleDesc"],
                    type = "range",
                    min = 0.75, max = 2, step = 0.05,
                    get = function()
                        return DragEmAll and DragEmAll.db.profile.flightMapScale or 1
                    end,
                    set = function(info, value)
                        if not DragEmAll then return end
                        DragEmAll.db.profile.flightMapScale = value
                        if DragEmAll.ApplyFlightMapScale then
                            DragEmAll:ApplyFlightMapScale()
                        end
                    end,
                    order = 30,
                },
                ...
                hidden = {
                    ...
                    args = {
                        ...
                        collapse_frames = {
                            ...
                            args = {
                                quest = {
                                    name = _G.QUESTS_LABEL or "Quests",
                                    type = "toggle",
                                    get = collapseFrameGet,
                                    set = collapseFrameSet,
                                    order = 10,
                                },
                                -- TBCC: Campaign, Adventure, Profession, Bonus, and World quests don't exist in TBCC
                                -- These features are Retail-only (Shadowlands+)
                                campaign = {
                                    name = _G.CONTAINER_CAMPAIGN_PROGRESS or "Campaign",
                                    type = "toggle",
                                    get = collapseFrameGet,
                                    set = collapseFrameSet,
                                    order = 20,
                                    hidden = not _G.CampaignQuestObjectiveTracker,
                                },
                                adventure = {
                                    name = _G.COVENANT_MISSIONS_TITLE or "Adventures",
                                    type = "toggle",
                                    get = collapseFrameGet,
                                    set = collapseFrameSet,
                                    order = 30,
                                    hidden = not _G.AdventureObjectiveTracker,
                                },
                                proffesion = {
                                    name = _G.PROFESSIONS_TRACKER_HEADER_PROFESSION or "Professions",
                                    type = "toggle",
                                    get = collapseFrameGet,
                                    set = collapseFrameSet,
                                    order = 40,
                                    hidden = not _G.ProfessionsRecipeTracker,
                                },
                                bonus = {
                                    name = _G.BONUS_OBJECTIVE_BANNER or "Bonus Objectives",
                                    type = "toggle",
                                    get = collapseFrameGet,
                                    set = collapseFrameSet,
                                    order = 50,
                                    hidden = not _G.BonusObjectiveTracker,
                                },
                                world = {
                                    name = _G.WORLD_QUEST_BANNER or "World Quests",
                                    type = "toggle",
                                    get = collapseFrameGet,
                                    set = collapseFrameSet,
                                    order = 60,
                                    hidden = not _G.WorldQuestObjectiveTracker,
                                },
                            },
                        },
                        collapse = {
