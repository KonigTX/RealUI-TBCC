...
local RavenTimer
-- TBCC: ZoneAbilityFrame may not exist, check before hooking
if _G.ZoneAbilityFrame and _G.ZoneAbilityFrame.Hide then
...
function RealUI:HuDTestMode(isConfigMode)
    ...
    -- TBCC: ObjectiveTrackerFrame doesn't exist in TBC Classic (use QuestWatchFrame)
    local questFrame = _G.ObjectiveTrackerFrame or _G.QuestWatchFrame
    if questFrame and not questFrame.collapsed then
        questFrame:SetShown(not isConfigMode)
    end
    ...
    -- Spell Alerts
    -- TBCC: SpellActivationOverlayFrame may not exist in Classic
    if _G.SpellActivationOverlayFrame then
        local sAlert = {
            id = 17941,
    ...
    -- Extra Action Button
    -- TBCC: ExtraActionBarFrame may not exist in Classic
    local EABFrame = _G.ExtraActionBarFrame
    if EABFrame and _G.HasExtraActionBar and not _G.HasExtraActionBar() then
        if isConfigMode then
    ...
    -- Zone Ability Button
    -- TBCC: ZoneAbilityFrame and C_ZoneAbility don't exist in TBCC
    local ZAFFrame = _G.ZoneAbilityFrame
    if ZAFFrame and _G.C_ZoneAbility then
        local zoneAbilities = _G.C_ZoneAbility.GetActiveAbilities()
        if ZAFFrame.SpellButtonContainer then
...
local function InitializeOptions()
    ...
    for i = 1, #tabs do
        ...
        if i == 1 then
            ...
            if Skin.UICheckButtonTemplate and type(Skin.UICheckButtonTemplate) == "function" then
